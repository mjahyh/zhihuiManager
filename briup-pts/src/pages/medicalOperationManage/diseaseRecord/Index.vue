 ...
 * @LastEditors: Ronda
 * @LastEditTime: 2022-04-17 21:31:53
-->
<template>
  <div>
    <!-- {{ authsButton }} -->
      ...
      <el-form :inline="true" size="small">
        ...
        <el-form-item>
          <el-select v-model="param.drDId" clearable placeholder="病症">
            <el-option v-for="item in diseaseData" :key="item.dId" :label="item.dName" :value="item.dId" />
          </el-select>
        </el-form-item>
    ...
    >
      <div slot="content">
        <el-form ref="d_record_form" :model="form" :rules="rules">
          <el-form-item label="动物编号" :label-width="formLabelWidth" prop="drAnimalId">
            <el-input
          ...
          <el-form-item label="病症" :label-width="formLabelWidth" prop="drDId">
            <el-select v-model="form.drDId" placeholder="请选择病症" @change="loadRecordMByRoleId">
              <el-option v-for="item in diseaseData" :key="item.dId" :label="item.dName" :value="item.dId" />
            </el-select>
          </el-form-item>
          <div v-if="roles.includes('admin')">
            ...
            <el-form-item label="治疗人员" :label-width="formLabelWidth" prop="drUdockerId">
          ...
          <el-form-item label="上报时间" :label-width="formLabelWidth" prop="drTime">
            <el-date-picker
              v-model="form.drTime"
              type="datetime"
              placeholder="选择上报时间"
          ...
          <el-form-item label="病症描述" :label-width="formLabelWidth" prop="drDesc">
            <el-input
              v-model="form.drDesc"
              type="textarea"
              :rows="2"
    ...
    >
      <div slot="content">
        <el-form ref="d_cure_record_form" :model="form" :rules="rules2">
          ...
          <el-form-item label="当前病症诊疗过程" :label-width="formLabelWidth" prop="drCure">
            <el-input
              v-model="form.drCure"
              type="textarea"
              :rows="2"
          ...
          <el-form-item :label-width="formLabelWidth" label="诊疗状态" prop="drStatus">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" size="small" @click="toSave2">确 定</el-button>
      </span>
    </Briupdrawer>
    <!-- 抽屉结束 -->
  </div>
...
<script>
import { findByOption, saveOrUpdate, delById, deleteByIdAll, findAllRecordAccount, findAllDisease } from '@/api/ptsOperational/diseaseRecord'
...
export default {
  ...
  methods: {
    ...
    toEdit(record) {
      this.loadDisease()
      this.loadAllRoles()
      this.loadRecordMByRoleId(record.account.baseRole.roleId, record.account.accountId)
      this.form = {
        ...record,
