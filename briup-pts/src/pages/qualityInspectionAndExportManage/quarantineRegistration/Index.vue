 ...
 * @LastEditors: Ronda
 * @LastEditTime: 2022-04-17 21:32:55
-->
<template>
  <div>
    <!-- {{ authsButton }} -->
      ...
      <el-table-column label="操作" align="center" width="140" fixed="right">
        <template slot-scope="scope">
          <el-button
            ...
            type="text"
            size="mini"
            @click.prevent="toDelete(scope.row.grId)"
          >移除</el-button>
          <el-button
            v-if="
              authsButton.filter((item) => {
      ...
      <div slot="content">
        <el-form ref="feed_form" :model="form" :rules="rules">
          ...
          <div v-if="roles.includes('admin')">
            <el-form-item label="角色(检疫人员)" :label-width="formLabelWidth" prop="roleId">
            ...
            <el-form-item label="检疫人员" :label-width="formLabelWidth" prop="grUquarantinerId">
              <el-select v-model="form.grUquarantinerId" placeholder="请选择检疫人员">
                <el-option v-for="item in quaMDataAll" :key="item.accountId" :label="item.username" :value="item.accountId" />
              </el-select>
            </el-form-item>
          </div>
        </el-form>
...
<script>
import { findByOption, saveOrUpdate, delById, deleteByIdAll, selectAllAccount } from '@/api/ptsOperational/quarantineRegistration'
  ...
  data() {
    return {
      ...
      rules: {
        ...
        grTime: [
          { type: 'string', required: true, message: '请选择检疫时间', trigger: 'change' }
        ],
        grImg: [
          { required: true, message: '请上传检疫登记图片', trigger: 'change' }
        ],
        bQualified: [
