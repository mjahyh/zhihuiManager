 ...
 * @LastEditors: Ronda
 * @LastEditTime: 2022-04-17 19:58:07
-->
<template>
  <div>
    <!-- {{ authsButton }} -->
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-popover size="mini" trigger="hover" placement="top">
            <img style="width:100px;height:100px" :src="scope.row.fImg" alt="">
            <div slot="reference" class="name-wrapper">
              <img style="width:50px;height:50px" :src="scope.row.fImg" alt="">
            </div>
          </el-popover>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" width="140" fixed="right">
        <template slot-scope="scope">
          <el-button
            v-if="scope.row.fDelete==0 && authsButton.filter((item) => {
    ...
    >
      <div slot="content">
        <el-form ref="feed_form" :model="form" :rules="rules">
          ...
          <el-form-item label="供应商" :label-width="formLabelWidth" prop="fSupplier">
            <el-input
              v-model="form.fSupplier"
              autocomplete="off"
            />
          </el-form-item>
          <el-form-item label="饲料类型" :label-width="formLabelWidth" prop="fType">
          ...
          <el-form-item label="价格" :label-width="formLabelWidth" prop="fPrice">
            <el-input
              v-model.number="form.fPrice"
              autocomplete="off"
            />
          </el-form-item>
          <!--图片（`图片上传单独接口`）  -->
          ...
          <el-form-item label="饲料图片地址" :label-width="formLabelWidth" prop="fImg">
          ...
          <el-form-item label="创建时间" :label-width="formLabelWidth" prop="fTime">
            <el-date-picker
              v-model="form.fTime"
              type="datetime"
              placeholder="选择日期时间"
...
<script>
import { findByOption, saveOrUpdate, delById, deleteByIdAll } from '@/api/ptsBase/feed'
  ...
  data() {
    return {
      ...
      rules: {
        ...
        fType: [
          { required: true, message: '请选择饲料类型', trigger: 'blur' }
        ],
        fPrice: [
        ...
        fTime: [
          { type: 'string', required: true, message: '请选择购入时间', trigger: 'change' }
        ],
        fImg: [
          { required: true, message: '请上传饲料图片', trigger: 'change' }
        ]
